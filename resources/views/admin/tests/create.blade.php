...
@php
    ...
    foreach ($problemBank as $p) {
        $initialPoints[$p->id] = (int) old('points.'.$p->id, 100);
        $allProblemsJson[] = [
            'id'         => (string) $p->id,
            'title'      => $p->title,
...
<div x-data="testForm({{ \Illuminate\Support\Js::from($initialPoints) }}, {{ \Illuminate\Support\Js::from($allProblemsJson) }})" class="max-w-4xl mx-auto space-y-5">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold text-gray-800">{{ __('admin.tests.create') }}</h1>
        <a href="{{ route('admin.tests.index') }}" class="text-sm text-gray-500 hover:text-gray-700">{{ __('common.back') }}</a>
    </div>

    <form method="POST" action="{{ route('admin.tests.store') }}" class="space-y-5">
        @csrf

        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.tests.title_label') }}</label>
                <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. JavaScript Mid-Level Screen"
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="time_limit_minutes" value="{{ old('time_limit_minutes', 60) }}" min="1" max="600"
                ...
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm @error('status') border-red-400 @enderror">
                        <option value="draft" @selected(old('status', 'draft') === 'draft')>Draft (not yet invitable)</option>
                        <option value="active" @selected(old('status') === 'active')>Active (invites can be created)</option>
                    </select>
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition">
                {{ __('admin.tests.create') }}
            </button>
            <a href="{{ route('admin.tests.index') }}" class="px-5 py-2 border border-gray-300 text-gray-600 text-sm rounded-lg hover:bg-gray-50 transition">
                {{ __('common.cancel') }}
            </a>
        </div>
    </form>
...
function testForm(initialPoints, allProblems) {
    return {
        search: '',
        showPicker: false,
        selected: @json(array_map('strval', old('problems', []))),
