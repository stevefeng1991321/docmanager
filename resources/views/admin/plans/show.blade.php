...
@section('content')
    ...
    @php
        ...
        $tc = ['todo'=>'bg-gray-100 text-gray-600','in_progress'=>'bg-blue-100 text-blue-700','completed'=>'bg-green-100 text-green-700','cancelled'=>'bg-red-100 text-red-700'];
    @endphp
                ...
                <div class="divide-y divide-gray-50">
                    ...
                    <div class="px-4 py-3 flex items-start gap-3 hover:bg-gray-50 transition" id="task-{{ $task->id }}">
                        ...
                        <form method="POST" action="{{ route('admin.plans.tasks.toggle', [$plan, $task]) }}" class="mt-0.5 flex-shrink-0">
                            @csrf @method('PATCH')
                            <button type="submit" class="w-5 h-5 rounded border-2 flex items-center justify-center transition {{ $task->status === 'completed' ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-green-400' }}">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-sm text-gray-800 {{ $task->status==='completed' ? 'line-through text-gray-400' : '' }}">{{ $task->title }}</span>
                                <span class="inline-flex px-1.5 py-0.5 rounded text-xs {{ $pc[$task->priority] ?? 'bg-gray-100 text-gray-500' }}">{{ ucfirst($task->priority) }}</span>
                                @if($task->is_overdue)
