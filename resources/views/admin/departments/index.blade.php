...
<div x-data="departmentManager()" class="space-y-5">
    ...
    <button @click="showForm = !showForm"
        ...
        + {{ __('admin.departments.new_department') }}
    </button>
    ...
    <div x-show="showForm" x-cloak class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <form method="POST" action="{{ route('admin.departments.store') }}" class="flex flex-wrap gap-3 items-end">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{ __('admin.departments.name_label') }}</label>
                <input type="text" name="name" required placeholder="{{ __('admin.departments.name_placeholder') }}"
            ...
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="description" placeholder="{{ __('common.description_placeholder') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-100 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{{ __('admin.departments.col_name') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{{ __('admin.departments.description_label') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{{ __('admin.departments.col_employees') }}</th>
            ...
            <tbody class="divide-y divide-gray-50">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-3">
                            <button type="button"
                                    ...
                                    class="text-xs text-blue-600 hover:text-blue-800">{{ __('admin.departments.edit_action') }}</button>
                            <form method="POST" action="{{ route('admin.departments.destroy', $dept) }}"
                                  onsubmit="return confirm('{{ __('admin.departments.confirm_delete', ['name' => '']) }}')">
                                @csrf @method('DELETE')
                                <button class="text-xs @if($dept->employees_count > 0) text-gray-300 cursor-not-allowed @else text-red-500 hover:text-red-700 @endif"
                                    ...
                                    {{ __('admin.departments.delete_action') }}
                                </button>
                            </form>
                        </div>
                    </td>
                ...
                @empty
                <tr><td colspan="4" class="px-4 py-8 text-center text-gray-400">{{ __('admin.departments.no_departments') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div x-show="editId !== null" x-cloak
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4" @click.stop>
            <h3 class="font-semibold text-gray-800">{{ __('admin.departments.edit') }}</h3>
            <form method="POST" x-bind:action="`{{ url('/admin/departments') }}/${editId}`" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">{{ __('admin.departments.description_label') }}</label>
