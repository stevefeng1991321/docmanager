...
@section('title', 'New Basic Knowledge Entry')

@section('content')
<div class="max-w-2xl mx-auto space-y-5">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold text-gray-800">{{ __('admin.basic_knowledge.create') }}</h1>
        <a href="{{ route('admin.basic-knowledge.index') }}" class="text-sm text-gray-500 hover:text-gray-700">{{ __('common.back') }}</a>
    </div>

    <form method="POST" action="{{ route('admin.basic-knowledge.store') }}"
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit"
                ...
                {{ __('admin.basic_knowledge.create') }}
            </button>
