@extends('layouts.app')
@section('title', __('science_tech.heading'))
...
<div class="space-y-6">
    ...
    <form method="GET" action="{{ route('science-tech.index') }}"
        ...
        <div class="relative w-full">
            <input type="text" name="q" value="{{ $q }}"
                   placeholder="{{ __('science_tech.search_placeholder') }}"
    ...
    @if($trends->isEmpty())
        <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p class="text-gray-500 font-medium">{{ __('science_tech.empty') }}</p>
            @if($q || $year)
                <p class="text-sm text-gray-400 mt-1">{{ __('science_tech.empty_sub') }}</p>
                <a href="{{ route('science-tech.index') }}" class="mt-3 inline-block text-sm text-blue-600 hover:underline">{{ __('common.clear') }}</a>
            @endif
        </div>
    @else
        ...
        <a href="{{ route('science-tech.show', $trend) }}"
           class="group bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all flex flex-col overflow-hidden">
            ...
            <div class="px-5 py-3 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
                ...
                <span class="text-xs text-blue-600 font-medium group-hover:underline">{{ __('science_tech.view_article') }} →</span>
            </div>
        </a>
        @endforeach
    </div>
